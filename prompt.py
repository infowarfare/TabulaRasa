...
def instruction_prompt(num_factual_elements: int) -> str:
    ...
    Identifiziere und liste alle relevanten Sachverhaltselemente auf. Fasse den Vortrag jeder Partei zu den jeweiligen Elementen präzise zusammen. Fasse den Vortrag jeder Partei wörtlich oder sinngemäß zusammen, ohne eigene Formulierungen hinzuzufügen. Berücksichtige besonders Punkte, die in einem Vortrag vorkommen und im anderen nicht. Markiere diese mit "-", um ein nicht vorhandenes Sachverhaltselement zu markieren. Identifiziere und liste die Beweismittel (z.B. Zeugen, Urkunden, Sachverständigengutachten, Dokumente jeglicher Art), die von jeder Partei für die jeweiligen Sachverhaltselemente angeboten werden."""
